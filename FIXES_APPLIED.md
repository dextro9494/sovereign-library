...
```markdown
- [Council](/council/)
- [v8 Three-Layer Architecture](/library/philosophy/v8-three-layer-architecture/)
```
...
2. **Verify after deployment:**
   - Test tag pages: ``https://dextro9494.github.io/sovereign-library (v3)/`tags/ai-council/`
...
## Phase 3 Auto-Fix (2026-01-15 17:55:09)

- Fixed invalid TOML line in `config.toml` (removed stray `= true`).
- Rewrote any remaining `sovereign-library (v2)` URLs to `sovereign-library (v3)` (0 remain).
- Updated legacy section links to new Phase 3 paths:
  - `/patterns/` → `/library/patterns/`
  - `/protocols/` → `/library/protocols/`
  - `/philosophy/` → `/library/philosophy/`
  - `/field-tests/` → `/library/field-tests/`
  - also updated relative `patterns/…` etc.
- Added standalone session docs to repo root:
  - `v3-phase3-quick-summary.md`
  - `v3-phase3-reorganization-complete.md`
  - `v3-phase4-next-session.md`
...
1) `config.toml`
- Removed stray invalid line: `= true`

2) Link rewrites (no v2 leakage)
- Replaced all occurrences of `sovereign-library (v2)` → `sovereign-library (v3)`

3) Phase 3 reorg link updates
- `/patterns/` → `/library/patterns/`
- `/protocols/` → `/library/protocols/`
- `/philosophy/` → `/library/philosophy/`
- `/field-tests/` → `/library/field-tests/`
- Also updated relative paths like `patterns/...` to `library/patterns/...` (and same for the other sections).

4) Added standalone markdown files to repo root
- `v3-phase3-quick-summary.md`
- `v3-phase3-reorganization-complete.md`
- `v3-phase4-next-session.md`
